...
def extraer_id_carpeta(valor: str) -> str:
    """Acepta tanto un ID de carpeta suelto como el link completo de
    Drive (con o sin parámetros ?usp=...) y siempre devuelve solo el
    ID."""
...
def get_drive_parent_folder_id() -> str:
    """Carpeta de Drive FIJA donde vivimos nosotros como equipo: ahí se
    guarda cada Sheet (uno por actividad) y su JSON crudo, para
    nuestro registro interno de análisis. No es algo que el profesor
    tenga que conocer ni escribir en el formulario -- se configura una
    sola vez aquí, vía variable de entorno."""
    raw = os.environ.get("DRIVE_PARENT_FOLDER_ID")
    if not raw:
        raise RuntimeError(
            "Falta la variable de entorno DRIVE_PARENT_FOLDER_ID "
            "(el ID o link de la carpeta de Drive donde se guardan los Sheets/JSON)."
        )
    return extraer_id_carpeta(raw)
...
def obtener_o_crear_subcarpeta(drive_service, carpeta_padre_id: str, nombre: str) -> str:
    """Busca una subcarpeta con ese nombre DENTRO de la carpeta padre;
    si no existe, la crea. Así cada actividad (Sheet + todos sus JSON)
    queda agrupada en su propia carpeta, en vez de todo suelto."""
    query = (
...
async def generate(req: ActividadRequest):
    ...
    try:
        carpeta_padre_id = get_drive_parent_folder_id()
        creds = get_google_credentials()
        gc = gspread.authorize(creds)
        drive_service = build("drive", "v3", credentials=creds)

        nombre_sheet = f"{req.actividad_numero} - {req.actividad_nombre} ({req.arranque})"
        carpeta_drive_id = obtener_o_crear_subcarpeta(drive_service, carpeta_padre_id, nombre_sheet)
        ws = obtener_o_crear_sheet(gc, drive_service, carpeta_drive_id, nombre_sheet)
    except Exception as e:
        # Cualquier error aquí (carpeta no encontrada, sin permisos,
        # credenciales inválidas, etc.) se devuelve como un error claro
        # en vez de dejar que la excepción tumbe la conexión a medias
        # -- eso es lo que antes se veía como "bloqueado por CORS" en
        # el navegador, cuando en realidad era un 500 sin manejar.
        raise HTTPException(
            status_code=500,
            detail=f"No se pudo acceder a la carpeta de Drive o crear el Sheet: {e}"
        )
            ...
            for idx, alumno in enumerate(req.alumnos, start=2):
                ...
                try:
                    ...
                    json_link = subir_json_crudo(
                        drive_service, carpeta_drive_id, alumno.nombre or f"fila_{idx}", resultado
                    )
